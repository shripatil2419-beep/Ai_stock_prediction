...
export class WatchlistService {
  ...
  refreshWatchlist() {
    this.http.get<{ok: boolean, watchlist: string[]}>(`${environment.apiUrl}/watchlist`, { headers: this.getHeaders() }).subscribe(res => {
      if (res.ok) this.watchlistSubject.next(res.watchlist);
    });
  }

  toggleWatchlist(symbol: string): Observable<any> {
    return this.http.post<any>(`${environment.apiUrl}/watchlist/toggle`, { symbol }, { headers: this.getHeaders() }).pipe(
      tap(res => {
        if (res.ok) this.watchlistSubject.next(res.watchlist);
      })
    );
